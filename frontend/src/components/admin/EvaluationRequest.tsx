...
const EvaluationRequest = () => {
    ...
    const fetchEvaluatorsList = async () => {
        try {
            const res = await axios.get('${BASE_URL}/admin/get-evaluators');
    ...
    const fetchEvaluators = async (status: 'pending' | 'get-evaluators') => {
        try {
            const res = await axios.get(`${BASE_URL}/evaluator/${status}`, {
    ...
    const handleStatusChange = async (id: string, status: 'approved' | 'rejected') => {
        try {
            await axios.put(`${BASE_URL}/evaluator/${status}/${id}`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
  ...
  setEditableList(updated);
    ...
    const handleSave = async (index: number) => {
        ...
        try {
            await axios.put(`${BASE_URL}/admin/evaluators/${evaluator._id}`, evaluator, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleRejectApproved = async (index: number) => {
        const evaluator = editableList[index];
        try {
            await axios.put(`${BASE_URL}/admin/evaluators/${evaluator._id}`, { ...evaluator, status: 'rejected' }, {
                headers: { Authorization: `Bearer ${token}` },
            });

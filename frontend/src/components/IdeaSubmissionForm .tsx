...
const IdeaSubmissionForm: React.FC = () => {
  ...
  const loadDraft = async () => {
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/schools/drafts/${udiseCode}`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
  ...
  const fetchGuideTeachers = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/school/get-registered-guide-teachers`, {
        headers: {
  ...
  const handleFinalSubmission = async ({
    ...
    try {
      ...
      const token = localStorage.getItem('schoolToken');
      await axios.delete(
        `${BASE_URL}/schools/drafts/${parsedSchoolDetails.udiseCode}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
